...
CREATE TABLE Componentes (
    id INT AUTO_INCREMENT PRIMARY KEY, -- Identificador único para cada componente
    tipo ENUM('GPU', 'CPU', 'RAM', 'PSU', 'SSD', 'Placa Base') NOT NULL, -- Tipo de componente: GPU, CPU, RAM, PSU, SSD, Placa Base
    nombre VARCHAR(255) NOT NULL, -- Nombre del componente
	imagen VARCHAR(255) NOT NULL, -- Imagen del Componente
    marca VARCHAR(255), -- Marca del componente
    descripcion TEXT, -- Descripción del componente
    precio DECIMAL(10, 2) NOT NULL, -- Precio del componente
    modelo VARCHAR(255), -- Modelo del componente
    vram VARCHAR(255), -- VRAM (Memoria de Acceso Aleatorio para Video) - Pertenece a GPU y SSD
    ...
    ancho_banda_memoria VARCHAR(255), -- Ancho de Banda de Memoria - Pertenece a GPU y SSD
    interfaz VARCHAR(255), -- Interfaz de Memoria - Pertenece a GPU y SSD
    consumo_energia DECIMAL(5, 2), -- Consumo de Energía - Pertenece a GPU, CPU y PSU
    compatibilidad_tecnologias TEXT, -- Compatibilidad con Tecnologías Específicas - Pertenece a GPU y CPU
    tamano_tipo_refrigeracion VARCHAR(255), -- Tamaño y Tipo de Refrigeración - Pertenece a GPU y CPU
    numero_nucleos_hilos INT, -- Número de Núcleos y Hilos - Pertenece a CPU
    cache_l3 VARCHAR(255), -- Caché L3 - Pertenece a CPU
    tecnologia_fabricacion VARCHAR(255), -- Tecnología de Fabricación - Pertenece a GPU y CPU
    soporte_instrucciones TEXT, -- Soporte para Instrucciones Específicas - Pertenece a CPU
    compatibilidad_plataformas_chipsets VARCHAR(255), -- Compatibilidad con Plataformas y Chipsets - Pertenece a CPU
    integracion_gpu VARCHAR(255), -- Integración Gráfica - Pertenece a CPU
    actualizabilidad VARCHAR(255), -- Actualizabilidad - Pertenece a CPU y PSU
    capacidad VARCHAR(255), -- Capacidad - Pertenece a RAM y SSD
    velocidad_lectura_escritura VARCHAR(255), -- Velocidad de Lectura/Escritura - Pertenece a RAM y SSD
    durabilidad VARCHAR(255), -- Durabilidad - Pertenece a SSD
    latencia VARCHAR(255), -- Latencia - Pertenece a RAM
    tipo_memoria VARCHAR(255), -- Tipo de Memoria - Pertenece a RAM y SSD
    canales_memoria VARCHAR(255), -- Canal Dual / Cuádruple - Pertenece a RAM
    voltaje VARCHAR(255), -- Voltaje - Pertenece a RAM
    potencia_nominal DECIMAL(5, 2), -- Potencia Nominal - Pertenece a PSU
    eficiencia_energetica VARCHAR(255), -- Eficiencia Energética - Pertenece a PSU
    conectores VARCHAR(255), -- Conectores - Pertenece a PSU
    modularidad VARCHAR(255), -- Modularidad - Pertenece a PSU
    protecciones VARCHAR(255), -- Protecciones - Pertenece a PSU
    ruido VARCHAR(255), -- Ruido - Pertenece a PSU
    compatibilidad VARCHAR(255), -- Compatibilidad - Pertenece a PSU y Placa Base
    factor_forma VARCHAR(255), -- Factor de Forma - Pertenece a Placa Base
    socket_cpu VARCHAR(255), -- Socket del Procesador - Pertenece a Placa Base
    chipset VARCHAR(255), -- Chipset - Pertenece a Placa Base
    numero_ranuras_ram INT, -- Número de Ranuras RAM - Pertenece a Placa Base
    slots_expansion VARCHAR(255), -- Slots de Expansión - Pertenece a Placa Base
    conectividad VARCHAR(255), -- Conectividad - Pertenece a Placa Base
    ram_soportada VARCHAR(255) -- RAM Soportada - Pertenece a Placa Base
);

-- Inserts para SSD


INSERT INTO Componentes (tipo, nombre, imagen, marca, descripcion, precio, modelo, capacidad, velocidad_lectura_escritura, durabilidad, tipo_memoria, factor_forma, interfaz)
VALUES 
...
INSERT INTO Componentes (tipo, nombre, imagen, marca, descripcion, precio, modelo, socket_cpu, chipset, numero_ranuras_ram, slots_expansion, conectividad, ram_soportada, factor_forma)
VALUES 
...
-- Inserts para CPU
INSERT INTO Componentes (tipo, nombre, imagen, marca, descripcion, precio, modelo, nucleos_procesamiento, frecuencia_reloj, consumo_energia, numero_nucleos_hilos, cache_l3, tecnologia_fabricacion, soporte_instrucciones, compatibilidad_plataformas_chipsets, integracion_gpu, actualizabilidad)
VALUES 
...
INSERT INTO Componentes (tipo, nombre, imagen, marca, descripcion, precio, modelo, vram, nucleos_procesamiento, frecuencia_reloj, ancho_banda_memoria, interfaz, consumo_energia, compatibilidad_tecnologias, tamano_tipo_refrigeracion, tecnologia_fabricacion)
VALUES 
...
INSERT INTO Componentes (tipo, nombre, imagen, marca, descripcion, precio, modelo, consumo_energia, actualizabilidad, potencia_nominal, eficiencia_energetica, conectores, modularidad, protecciones, ruido, compatibilidad)
VALUES 
...
INSERT INTO Componentes (tipo, nombre, imagen, marca, descripcion, precio, modelo, capacidad, velocidad_lectura_escritura, durabilidad, tipo_memoria, canales_memoria, voltaje)
VALUES 
